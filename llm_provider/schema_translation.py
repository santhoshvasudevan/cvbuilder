"""Per-provider JSON-schema translation (requirements.md LLM-007).

Pydantic models are the canonical structured-output contract. Provider-specific schema
translation belongs entirely inside this app -- pipeline/domain code must never contain
Gemini/OpenAI/NVIDIA/OpenRouter-specific schema workarounds.
"""
...
def strip_enums(schema: Any) -> Any:
    ...
    Accumulating too many `enum` constraints across schema fields can cause opaque 400 errors
    from Gemini. The safe pattern is to drop `enum` constraints from the schema sent to Gemini
    and instead re-validate the returned values against the allowed set server-side afterward --
    `BaseLLMAdapter.generate()` does this uniformly via `output_schema.model_validate(...)`.
    """
...
def to_openai_strict_schema(model: type[BaseModel]) -> dict[str, Any]:
    ...
    OpenAI and NVIDIA NIM/OpenRouter's OpenAI-compatible endpoints support `$ref`/`$defs` and
    `enum` directly, so those are left as-is; strict mode additionally requires every object
    node to set `additionalProperties: false`.
    """
    raw = model.model_json_schema()
