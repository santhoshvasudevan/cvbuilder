...
Closing this decision's own risk: `job_applications.tests.test_models.ReasoningLevelVocabularyDriftTests` (added in the M2 correction, see V2-D043) asserts `set(_REASONING_LEVEL_VALUES) == set(llm_provider.models.ReasoningLevel.values)` directly, so a future change to one set without the other now fails a test rather than silently drifting.

---

## M2 Independent Re-Audit Correction

An independent re-audit of M2 (range `a7921b9..a9a9494`) returned **M2 RE-AUDIT FAILED — CORRECTION REQUIRED**, with one BLOCKER, one MAJOR, and one MINOR finding, plus one documentation-accuracy observation. V2-D042 and V2-D043 below record the two architectural additions the correction required; the correction itself is a direct child of `a9a9494` (see `docs/CURRENT_STATE.md` for the verified HEAD).

## V2-D042 — Temperature is explicit, model-driven capability metadata (`LLMModel.supports_temperature` / `supports_temperature_with_reasoning`), validated pre-flight like every other call parameter
**Status:** APPROVED

The M2 audit found (MAJOR) that `temperature` was accepted into every adapter's request body with no pre-flight validation at all -- unlike every other call parameter (provider/model active, credential, structured output, reasoning level, output budget), an incompatible `temperature` value reached a live HTTP call before failing, contradicting the "fails before HTTP" architecture goal `docs/ARCHITECTURE.md` Section 12's M2 implementation note already claimed was complete.

Closed by adding two explicit `LLMModel` boolean fields (`llm_provider/migrations/0002_llmmodel_supports_temperature_and_more.py`):

- `supports_temperature` -- whether this model accepts a `temperature` request parameter at all.
- `supports_temperature_with_reasoning` -- whether `temperature` may be combined with a non-`NONE` `reasoning_level` for this model; only consulted when `supports_temperature` is `True` and a non-`NONE` reasoning level is requested.

Both default to `True` (most models accept temperature unconditionally; the narrower cases are the exception, set explicitly per model in the registry). This is explicit capability metadata on the model row, never inferred from `model_identifier`/provider name string matching -- no `gpt-*`/`gemini-*`/reasoning-tier slug pattern appears anywhere in `llm_provider.validation`.

A canonical permitted numeric range, `llm_provider.models.MIN_TEMPERATURE`/`MAX_TEMPERATURE` (`0.0`–`2.0`, matching the range OpenAI/Gemini/OpenRouter all document for this parameter), is enforced unconditionally regardless of per-model capability.

`llm_provider.validation.validate_temperature_supported` is the single implementation of all three checks (range, support, reasoning-combination) and is wired into `validate_call_configuration` -- the same fixed-order, pre-flight entrypoint every other check already used, called from both the routing entrypoint (`llm_provider.adapters.get_adapter_for_stage`, using `StageModelAssignment.default_temperature`) and defensively again inside `BaseLLMAdapter.generate()` (using the per-call `NormalizedLLMRequest.temperature`), so a runtime override is checked by the exact same code path as the stored default. `StageModelAssignment.clean()` enforces the identical rule at save time, so an invalid assignment cannot even be persisted through the admin. Three new typed `ConfigurationError` subclasses (`TemperatureOutOfRangeError`, `UnsupportedTemperatureError`, `TemperatureReasoningConflictError`) keep the failure classified as `CONFIGURATION`, never `SCHEMA_VALIDATION` or a transient provider category.

## V2-D043 — Gemini credential moves to the documented `x-goog-api-key` header; `sanitize_error_message` hardened as a defence-in-depth invariant against URL/query-string credential leakage
**Status:** APPROVED

The M2 audit found (BLOCKER) that `llm_provider.adapters.gemini.GeminiAdapter` placed the API key directly in the request URL's query string (`?key=...`) -- the only one of the four adapters to do so; OpenAI/NVIDIA NIM/OpenRouter all send their credential via an `Authorization: Bearer` header. `requests`/`urllib3` connection-level exceptions (`ConnectionError`, `Timeout`, `SSLError`, etc.) stringify with the full request URL, and the pre-correction `sanitize_error_message` only redacted `{...}`/`[...]`-shaped substrings -- a query-string credential passed through completely unredacted into the persisted, admin-viewable `LLMCallLog.error_message` on any transient network failure. Verified empirically (`requests.post` against an unreachable host with a synthetic key embedded in the URL, run through the actual pre-correction `sanitize_error_message`) before any fix was written.

Closed with defence in depth, per official Gemini API documentation (`https://ai.google.dev/gemini-api/docs/api-key`, which documents `x-goog-api-key` as a supported REST authentication header):

1. **Primary fix** -- `GeminiAdapter` sends the credential via the `x-goog-api-key` header; the request URL never contains it. The request URL is asserted, by test, to contain no `?` at all.
2. **Backstop 1** -- `llm_provider.errors.sanitize_error_message` now redacts, case-insensitively, sensitive URL/query-string parameters (`key`, `api_key`, `apikey`, `token`, `access_token`, `auth`, `authorization`, `password`, `secret`, `signature`) wherever they appear -- inside a full URL, a bare path+query fragment (the exact shape a `urllib3` connection error produces, with no scheme), URL-encoded values, multiple parameters in any position, URL user-info (an `https://<name>:<secret>@host`-shaped prefix), and `Bearer`/`Basic ` auth-scheme-prefixed tokens -- independent of whether any adapter still needs it. This exists specifically so a *future* adapter or a change to an existing one cannot reintroduce this class of leak silently.
3. **Backstop 2** -- adapters no longer pass raw `requests.RequestException`/`requests.Timeout` text through `sanitize_error_message` at all for the actual HTTP-call boundary. `NormalizedLLMError.from_network_exception` (new) classifies the exception by type only (`llm_provider.errors.classify_network_exception`) into a short, safe, provider-agnostic message (e.g. "Connection error contacting the provider.") -- the raw exception text, and therefore anything it might contain, is never a candidate for the stored/raised message in the first place. `from_exception` (unchanged) remains available for non-network exceptions (e.g. schema-validation `ValidationError`s from parsing a provider's response body), where the redaction backstop above is the only defence needed.

None of this changes retryability classification (`LLMErrorCategory`/`TRANSIENT_ERROR_CATEGORIES` are untouched), the audit ledger's schema (no new `LLMCallLog` field), or introduces any provider/model fallback -- `classify_network_exception` only changes what text describes a failure, never which provider/model a retry targets.

---

## M2 Second Independent Re-Audit Correction

A second independent re-audit of the M2 line (range `a7921b9..5e6a40c`) found the original BLOCKER, MAJOR, and MINOR findings genuinely closed, but identified one new MAJOR and two new MINOR findings in the V2-D042 correction itself. This entry (V2-D044) records the Product Owner-directed fix, append-only; it does not rewrite V2-D042 or V2-D043.

## V2-D044 — Temperature capability flags are fail-closed by default; `supports_temperature_with_reasoning=True` requires `supports_temperature=True`
**Status:** APPROVED (Product Owner decision)

The second re-audit found (MAJOR) that V2-D042's `LLMModel.supports_temperature`/`supports_temperature_with_reasoning` both defaulted to `True` -- fail-open, inconsistent with the sibling capability fields on the identical model row (`supports_structured_output` defaults `False`; `supported_reasoning_levels` defaults `[NONE]`, i.e. no reasoning capability assumed). This was empirically demonstrated to matter, not merely theoretical: a pre-existing `LLMModel` row migrated through `0002_llmmodel_supports_temperature_and_more` with no operator action received `supports_temperature=True` and `supports_temperature_with_reasoning=True` -- silently granting a capability, for a real class of models (OpenAI's reasoning-tier models, which reject `temperature` outright) that never had it reviewed.

The Product Owner decision: **temperature capability must be fail-closed and explicitly enabled per model.** Closed as follows:

1. `LLMModel.supports_temperature` and `supports_temperature_with_reasoning` now both default `False`. A model may use `temperature` only after explicit per-model opt-in -- consistent with `supports_structured_output`/`supported_reasoning_levels`'s existing fail-closed pattern.
2. `llm_provider/migrations/0002_llmmodel_supports_temperature_and_more.py` was edited directly (not amended via a follow-up migration) to carry `default=False` for both fields. This is safe because migration `0002` was confirmed, before editing, to be **unpublished**: it was introduced only in the first correction commit (`5e6a40c`), which has never been merged into `cvbuild2` (still at `a7921b9`), never pushed to any remote (`origin` has no branch containing it), and has only ever been applied to disposable, isolated PostgreSQL containers created and destroyed during audit sessions -- never a persistent or shared V2 database. No intentional operator configuration could have been overwritten, because none has ever existed against this migration's old defaults outside a disposable test resource. Re-verified empirically: a representative pre-existing `LLMModel` row (reasoning-capable, created before `0002` is applied) now receives `supports_temperature=False`/`supports_temperature_with_reasoning=False` on migration -- fail-closed, not fail-open.
3. `LLMModel.clean()` now rejects `supports_temperature_with_reasoning=True` combined with `supports_temperature=False` -- the internally-inconsistent registry state the second re-audit's MINOR-1 finding identified (previously representable, though inert, because downstream checks always tested `supports_temperature` first). This closes it at the source-of-truth model level, not just downstream at every consumer.
