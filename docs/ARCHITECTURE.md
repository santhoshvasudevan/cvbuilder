...
**M2 correction note (V2-D042):** `LLMModel` carries two additional explicit capability fields beyond this section's base list — `supports_temperature` and `supports_temperature_with_reasoning` (both default `True`) — so `temperature` is validated pre-flight the same way every other call parameter already was, via `llm_provider.validation.validate_temperature_supported`. A canonical numeric range (`MIN_TEMPERATURE`/`MAX_TEMPERATURE`, 0.0–2.0) is enforced unconditionally. This is model-driven capability metadata, never inferred from a model identifier or provider name.
