...
| MEM-003 MemoryClaim provenance (v1.1: hash + quote + start/end line, not quote-only, per D-003) | §4 | candidate_memory | M3 | MemoryClaim.source_document FK + MemorySourceDocument.content_sha256 + quote + source_start_line/source_end_line | unit test: every claim has a resolvable source pointer with hash and line range | Not implemented |
| MEM-004 confirmation_status unconfirmed/confirmed/retired; only confirmed usable downstream | §4 | candidate_memory | M3 | MemoryClaim.confirmation_status + retrieval filter | unit test: unconfirmed/retired claims excluded from AC retrieval | Not implemented |
| MEM-005 no fabrication — organize/phrase only, never invent | §4 | candidate_memory | M3 | memory-build prompt + validator | unit test: claim text must be traceable to source excerpt | Not implemented |
| MEM-006 memory versioned; new markdown → new revision; old revisions kept (v1.1: snapshot + incremental processing per D-002, not full reprocess) | §4 | candidate_memory | M3 | CandidateMemory.version + content-hash-based revision service | unit test: unchanged documents are not reprocessed and may carry confirmed status forward; changed/new documents are reprocessed and start unconfirmed; old version rows untouched | Not implemented |
...
| DATA-002 MemorySourceDocument | §8 | candidate_memory | M3 | model | migration + admin check | Not implemented |
| DATA-003 MemoryClaim | §8 | candidate_memory | M3 | model | migration + admin check | Not implemented |
