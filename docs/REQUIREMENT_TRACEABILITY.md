...
Status: Milestones M1, M2, and M3 are implemented and verified (see `docs/CURRENT_STATE.md` for
detail); M4 onward remain not implemented. Every row below is truthful as of this writing — a row
...
| Requirement | Source | Component | Milestone | Artifact | Verification | Status |
|---|---|---|---|---|---|---|
| AJ-001 input via URL or pasted text, both required | §5 | job_intake | M4 | `forms.py`/`services/intake.py::resolve_posting_source` | `test_intake_service.py`, `test_views.py` (both paths, both-supplied/neither-supplied validation errors) | **Implemented, uncommitted** — FakeAdapter-verified only, not live-provider-verified |
| AJ-002 detect/work in posting's own language | §5 | job_intake | M4 | AJ prompt (`services/analyze.py`) + `posting_language` field | `test_intake_service.py::test_german_posting_is_analyzed_in_its_own_language` (fixture, not live) | **Implemented, uncommitted** — prompt instructs language detection; real non-English live-model behavior not verified (no live call made) |
...
| AJ-004 identify mandatory/preferred reqs, responsibilities, ATS keywords, screening risks, implied expectations | §5 | job_intake | M4 | `schemas.py::AgentJobberAnalysis`/`RequirementCategory` | `test_schemas.py`, `test_intake_service.py` (all five categories + screening_risks round-trip) | **Implemented, uncommitted** |
| AJ-005 output tagged source_type + original raw text/URL | §5 | job_intake | M4 | `JobRequirementAnalysis` model | `test_intake_service.py::test_pasted_text_preserves_exact_operator_input`, `test_models.py` | **Implemented, uncommitted** |
| AJ-007 (v1.1/D-014) stable JobRequirement IDs (JR-001, ...) with category, stable within a JRA version | §5, §16 | job_intake | M4 | `JobRequirement` model + `services/intake.py::run_intake` | `test_models.py::JobRequirementIdAndOrderingTests`, `test_intake_service.py::test_model_cannot_control_canonical_jr_ids` | **Implemented, uncommitted** |
| AJ-006 URL fetch best-effort; always allow pasted-text fallback; UI surfaces fetch failure | §5 | job_intake | M4 | `services/fetch.py` + `views.py` | `test_fetch.py` (14 tests, all mocked), `test_views.py::IntakeViewUrlFetchFailureTests` | **Implemented, uncommitted** — only mocked HTTP responses exercised; no real-world URL fetch verified |
...
| DATA-004 JobRequirementAnalysis (+ child JobRequirement, v1.1/D-014) | §8, §16 | job_intake | M4 | model | migration applied against real PostgreSQL; `test_models.py` (version uniqueness, append-only, stable IDs) | **Implemented, uncommitted** |
...
| Requirement | Source | Component | Milestone | Artifact | Verification | Status |
|---|---|---|---|---|---|---|
| TRACE-001 Agent Jobber assigns stable JobRequirement IDs (JR-001, ...) with category, stable within a JRA version | §16 | job_intake | M4 | JobRequirement model | `test_models.py::JobRequirementIdAndOrderingTests` | **Implemented, uncommitted** |
