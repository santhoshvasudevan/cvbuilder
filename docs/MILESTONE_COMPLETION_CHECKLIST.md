...
An independent re-audit of M2 (range `a7921b9..a9a9494`) returned **M2 RE-AUDIT FAILED — CORRECTION REQUIRED**: one BLOCKER, one MAJOR, one MINOR finding, and one documentation-accuracy observation. All four are closed in a corrective commit that is a direct child of `a9a9494` (see `docs/CURRENT_STATE.md` for the verified HEAD; full rationale in `docs/DECISIONS.md`'s "M2 Independent Re-Audit Correction" section, V2-D042/V2-D043):

- [x] **BLOCKER closed** — Gemini adapter no longer places its credential in the request URL/query string; it uses the documented `x-goog-api-key` header (verified against `https://ai.google.dev/gemini-api/docs/api-key`). `llm_provider.errors.sanitize_error_message` additionally redacts sensitive URL/query-string parameters, URL user-info, and auth-scheme tokens as a defence-in-depth invariant; adapters classify network-boundary exceptions by type (`NormalizedLLMError.from_network_exception`) instead of passing raw exception text through. Closed by an empirically-verified regression: a simulated connection failure whose exception text embeds a synthetic, credential-bearing URL is proven, by test, to leave the credential out of the raised error, `LLMCallLog.error_message`, and every other field of the persisted audit-log row (`llm_provider/tests/test_real_adapter_config_guards.py::GeminiAdapterTests`, five failure-mode tests: connection error, DNS, TLS, connection-refused, timeout).
- [x] **MAJOR closed** — `temperature` is now validated pre-flight (before any HTTP call) against explicit, model-driven capability metadata (`LLMModel.supports_temperature`/`supports_temperature_with_reasoning`, migration `llm_provider/migrations/0002_llmmodel_supports_temperature_and_more.py`) and a canonical numeric range (`MIN_TEMPERATURE`/`MAX_TEMPERATURE` = 0.0/2.0), via `llm_provider.validation.validate_temperature_supported`, wired into the same `validate_call_configuration` entrypoint every other pre-flight check already used — enforced identically at `StageModelAssignment.clean()` (save time) and at both the routing entrypoint and `BaseLLMAdapter.generate()` (call time, covering runtime overrides). No model slug/provider-name string matching anywhere in the validation logic.
- [x] **MINOR closed** — `job_applications/tests/test_models.py::ReasoningLevelVocabularyDriftTests` asserts `set(_REASONING_LEVEL_VALUES) == set(llm_provider.models.ReasoningLevel.values)` directly.
- [x] **Documentation observation closed** — `docs/TEST_STRATEGY.md`'s M2 section no longer claims blanket "error sanitization" coverage; it now states precisely what was (JSON/dict-body redaction) and was not (URL/query-string credential redaction) covered before this correction, and what the correction adds.
- [x] Migrations checked — the new migration applies cleanly to a fresh, isolated, disposable PostgreSQL database; a second `migrate` run is a no-op; `llm_provider` migrations unapply/reapply cleanly; `makemigrations --check --dry-run` reports no drift. The operational database/volume was never started or touched.
- [x] Tests — 220/220 pass (52 `job_applications` + 168 `llm_provider`, +49 tests over the pre-correction 171), all closing one of the findings above or guarding against its regression.
- [x] `make verify`/`make lint`/`make secrets`/`git diff --check` all pass on the corrected tree.
- [x] Zero live provider calls — every new/changed test mocks `requests.post` or constructs exceptions directly; no synthetic credential used in any test is a real credential, and no test asserts a credential is *present* in output, only that it is absent.
- [x] No M3 code introduced — this correction touches only `llm_provider`, `job_applications` tests, and canonical docs.
- [x] Docs updated — `docs/DECISIONS.md` (V2-D042, V2-D043, append-only), `docs/ARCHITECTURE.md` §12 (M2 correction implementation notes), `docs/CURRENT_STATE.md`, `docs/TEST_STRATEGY.md`, `docs/REQUIREMENT_TRACEABILITY.md`, this checklist.
- [x] Commit created only if authorized — authorized explicitly by the correction task's instructions, as a direct child of `a9a9494` on `m2-llm-provider-foundation`, never on `cvbuild2` or `main` directly; not merged, not pushed.

### M2 — Second Independent Re-Audit Correction

A second independent re-audit of the M2 line (range `a7921b9..5e6a40c`) confirmed the first correction's BLOCKER/MAJOR/MINOR genuinely closed, but found one new MAJOR and two new MINORs in that correction's own temperature-capability fix. All three are closed in a corrective commit that is a direct child of `5e6a40c` (see `docs/CURRENT_STATE.md` for the verified HEAD; full rationale in `docs/DECISIONS.md` V2-D044):

- [x] **MAJOR closed (Product Owner decision)** — `LLMModel.supports_temperature`/`supports_temperature_with_reasoning` now default `False` (fail-closed) instead of `True`. A model may use `temperature` only after explicit per-model opt-in, matching the existing `supports_structured_output`/`supported_reasoning_levels` pattern. Migration `0002_llmmodel_supports_temperature_and_more.py` was edited directly (verified unpublished first: never merged into `cvbuild2`, never pushed to any remote, only ever applied to disposable test databases) to carry the corrected default, so a fresh V2 database is fail-closed from the start. A representative pre-existing `LLMModel` row was empirically tested through the migration and confirmed to receive `False`/`False`, not a silent capability grant.
- [x] **MINOR closed** — `LLMModel.clean()` now rejects `supports_temperature_with_reasoning=True` combined with `supports_temperature=False`, closing the previously-representable (though inert) internally-inconsistent registry state.
- [x] **MINOR closed** — `OpenAIAdapterTests.test_invalid_temperature_never_reaches_http` proves, with `requests.post` mocked, that an invalid temperature configuration never reaches HTTP, triggers no retry, and is classified `CONFIGURATION`.
- [x] Additional coverage — `SaveTimeCallTimeParityTests` proves `StageModelAssignment.clean()` (save time) and `validate_temperature_supported` (call time, covering runtime overrides) agree on every tested configuration, so neither path can accept what the other would reject.
- [x] Migrations checked — corrected migration applies cleanly to a fresh, isolated, disposable PostgreSQL database; a second `migrate` run is a no-op; unapply/reapply clean; `makemigrations --check --dry-run` reports no drift. The operational database/volume was never started or touched.
- [x] Tests — 231/231 pass (52 `job_applications` + 179 `llm_provider`, +11 tests over the pre-this-correction 220), all closing one of the findings above or guarding against its regression. Existing Gemini credential-leak and reasoning-vocabulary drift tests confirmed still green.
- [x] `make verify`/`make lint`/`make secrets`/`git diff --check` all pass on the corrected tree.
- [x] Zero live provider calls — every new/changed test mocks `requests.post` or operates purely at the validation-function/model layer.
- [x] No M3 code introduced — this correction touches only `llm_provider` (models, migration, tests) and canonical docs.
- [x] No model-slug/provider-name special-casing introduced anywhere in the fix.
- [x] Docs updated — `docs/DECISIONS.md` (V2-D044, append-only, does not rewrite V2-D042/V2-D043), `docs/ARCHITECTURE.md` §12 (default corrected in the existing M2 correction note), `docs/CURRENT_STATE.md`, `docs/TEST_STRATEGY.md`, `docs/REQUIREMENT_TRACEABILITY.md`, this checklist.
- [x] Commit created only if authorized — authorized explicitly by the correction task's instructions, as a direct child of `5e6a40c` on `worktree-m2-correction`, never on `cvbuild2` or `main` directly; not merged, not pushed, no Git history rewritten.
